...
unsigned int  loadTexture( char const* textureLoad)
{
    unsigned int textureID;
    glGenTextures(1, &textureID);
    int width, height, nrChannels;
    unsigned char * data = stbi_load(textureLoad, &width, &height, &nrChannels, 0);
    if (data)
    {
        GLenum format;
        if (nrChannels == 1)
...
int main()
{
...
#ifdef __APPLE__
    glfwWindowHint(GLFW_OPENGL_FORWARD_COMPATmGL_TRUE);
    ...
    Shader lightingShader("E:\\vstudioproject\\LearnOpenGL\\LearnOpenGL\\vertexShaderSource_lightcast.GLSL", "E:\\vstudioproject\\LearnOpenGL\\LearnOpenGL\\fragmentShaderSource_lightcast1.GLSL");
